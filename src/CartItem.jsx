...
const CartItem = ({ onContinueShopping }) => {
  ...
  const calculateTotalAmount = (cart) => {
    ...
    cart.forEach(item => {
        ...
        total += item.quantity * itemCost; // Add the item's total cost to the total
    });
    return total;
  ...
  const handleIncrement = (dispatch, item) => {
    dispatch(updateQuantity({ name: item.name, quantity: item.quantity + 1 }));

  };

  const handleDecrement = (dispatch, item) => {
   if (item.quantity > 0) {
        dispatch(updateQuantity({ name: item.name, quantity: item.quantity - 1 }));
    } else {
  ...
  const handleRemove = (dispatch, item) => {
